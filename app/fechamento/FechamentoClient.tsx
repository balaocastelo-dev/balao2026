...
export default function FechamentoClient() {
  ...
  // Auth Handler
  // ---- restauração do histórico perdido na migração do banco ----
  //
  // As ordens de serviço de janeiro a agosto/2026 sumiram ao trocar de banco.
  // O backup do banco antigo está em disco; aqui o arquivo é lido no navegador
  // e enviado para gravação. Rodar duas vezes não duplica: cada registro tem
  // id próprio e é regravado por cima.
  const [restaurando, setRestaurando] = useState(false);

  const restaurarBackup = async (arquivos: FileList | null) => {
    if (!arquivos || arquivos.length === 0) return;
    setRestaurando(true);

    try {
      const pacote: { ordens: unknown[]; despesas: unknown[] } = { ordens: [], despesas: [] };

      for (const arquivo of Array.from(arquivos)) {
        const conteudo = JSON.parse(await arquivo.text());
        if (!Array.isArray(conteudo)) continue;
        // O nome do arquivo diz o que é: weekly_orders.json / weekly_expenses.json
        if (/expense|despesa/i.test(arquivo.name)) pacote.despesas.push(...conteudo);
        else pacote.ordens.push(...conteudo);
      }

      if (pacote.ordens.length === 0 && pacote.despesas.length === 0) {
        showToast("Nenhum registro encontrado nos arquivos escolhidos.");
        return;
      }

      const res = await fetch("/api/weekly/importar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        showToast(dados?.error || "Falha ao restaurar.");
        return;
      }

      showToast(
        `Restaurado: ${dados.ordensGravadas} ordem(ns) e ${dados.despesasGravadas} despesa(s).` +
          (dados.totalDeProblemas ? ` ${dados.totalDeProblemas} com problema.` : "")
      );
      if (dados.problemas?.length) console.warn("[fechamento] problemas:", dados.problemas);

      window.location.reload();
    } catch (erro) {
      showToast(`Não consegui ler o arquivo: ${(erro as Error).message}`);
    } finally {
      setRestaurando(false);
    }
  };

  // O portão de senha que existia aqui era só do navegador: a "senha do dia"
  // (56676009 + data) estava no JavaScript da página, visível para qualquer um
  // que abrisse o console — e as rotas `/api/weekly/*` nem isso exigiam. Agora
  // quem tranca é o servidor, com a senha do painel, antes desta tela existir.
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 no-print">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            {/* Restaurar backup do banco antigo. Aparece só quando não há nada
                na tela — é operação de recuperação, não de uso diário. */}
            {orders.length === 0 && expenses.length === 0 && (
              <label
                className={`flex items-center gap-2 rounded-lg border border-dashed border-blue-300 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 ${
                  restaurando ? "opacity-60" : "cursor-pointer hover:bg-blue-100"
                }`}
                title="Escolha weekly_orders.json e weekly_expenses.json do backup"
              >
                {restaurando ? "Restaurando…" : "⟲ Restaurar histórico do backup"}
                <input
                  type="file"
                  ...
                  onChange={(e) => restaurarBackup(e.target.files)}
                />
              </label>
            )}
