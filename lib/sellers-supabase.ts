...
export async function getSellersAtivos(): Promise<SellerDB[]> {
  const { data, error } = await supabase.from("sellers").select("id, name, slug, senha, ativo, cargo, photo").eq("ativo", true).order("name", { ascending: true });
...
export async function getSellerBySlug(slug: string): Promise<SellerDB | null> {
  ...
  if (!alvo) return null;
  const { data, error } = await supabase.from("sellers").select("id, name, slug, senha, ativo, cargo, photo").eq("slug", alvo).eq("ativo", true).limit(1).maybeSingle();
...
export async function createSellerDB(input: { name: string; slug: string; senha: string; cargo?: string }): Promise<SellerDB | null> {
  ...
  const { data, error } = await supabase.from("sellers").insert({ name: input.name.trim(), slug, senha: input.senha.trim(), cargo: input.cargo || "Vendas", ativo: true, hired_at: new Date().toISOString().slice(0,10) }).select("id, name, slug, senha, ativo, cargo").single();
...
export async function updateSellerDB(id: string, updates: Partial<Pick<SellerDB,"name"|"slug"|"senha"|"cargo"|"ativo">>): Promise<SellerDB | null> {
  ...
  const { data, error } = await supabase.from("sellers").update(payload).eq("id", id).select("id, name, slug, senha, ativo, cargo").single();
  if (error) throw error;
  return data as SellerDB;
}
