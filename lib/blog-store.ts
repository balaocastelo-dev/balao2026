  ...
  if (isSupabaseReadable()) {
    ...
    if (dbPosts.length > 0) {
      const mapped = dbPosts.map((p) => ({
        ...
        category: normalizeCategory(p.category, { title: p.title, sourceUrl: p.source_url ? String(p.source_url) : undefined }),
  ...
  if (isSupabaseReadable()) {
    ...
    if (post) {
      return {
        ...
        category: normalizeCategory(post.category, { title: post.title, sourceUrl: post.source_url ? String(post.source_url) : undefined }),
