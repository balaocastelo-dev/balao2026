...
import { Cpu, Laptop, ShieldCheck, Flame, CheckCircle2, ChevronRight, MessageCircle, Star, Sparkles, Gamepad2 } from "lucide-react";
import { Product, getProductHref } from "@/lib/utils";
import { SITE_CONFIG } from "@/lib/config";

interface HomeSeminovosShowcaseProps {
  pcGamerProducts: Product[];
  notebookProducts: Product[];
}

export default function HomeSeminovosShowcase({ pcGamerProducts, notebookProducts }: HomeSeminovosShowcaseProps) {
  const [activeTab, setActiveTab] = useState<"pcgamer" | "notebooks">("pcgamer");

  const currentList = activeTab === "pcgamer" ? pcGamerProducts : notebookProducts;
  const categoryLink = activeTab === "pcgamer" ? "/pcgamer" : "/categoria/notebooks-seminovos";
  const categoryTitle = activeTab === "pcgamer" ? "PC Gamer & Desktop de Alta Performance" : "Notebooks & MacBooks Seminovos";
  ...
  return (
    <section className="my-8 rounded-[2rem] border border-[var(--home-border)] bg-[var(--home-panel-bg)] p-4 sm:p-6 shadow-2xl relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute -top-20 -left-20 w-80 h-80 bg-red-500/10 rounded-full blur-3xl" />
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-white/10">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-red-500/20 text-red-400 border border-red-500/30 text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
              <Sparkles size={13} />
              Destaques em Campinas
            </span>
          </div>
          <h2 className="text-xl sm:text-3xl font-black text-white tracking-tight mt-1.5">
            Vitrines de Alta Performance: PC Gamer & Notebooks
          </h2>
          <p className="text-xs sm:text-sm text-[var(--home-muted)] mt-1">
            Máquinas montadas e testadas com garantia de bancada, procedência e retirada imediata no Cambuí.
          </p>
        </div>
        ...
        <div className="flex items-center p-1 rounded-2xl bg-black/40 border border-white/10">
          ...
          >
            <Gamepad2 size={16} />
            <span>PC Gamer ({pcGamerProducts.length})</span>
          </button>
      ...
      <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-2 my-4 py-2 px-3 rounded-2xl bg-white/[0.02] border border-white/5 text-[11px] text-[var(--home-soft)] font-bold">
        <div className="flex items-center gap-2">
          ...
          <span>Montagem & Testes de FPS</span>
        </div>
        <div className="flex items-center gap-2">
          <ShieldCheck size={15} className="text-blue-400 shrink-0" />
          <span>Garantia Balão de 90 dias a 1 ano</span>
        </div>
        <div className="flex items-center gap-2">
          <Sparkles size={15} className="text-amber-400 shrink-0" />
          <span>Componentes 100% Originais</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle2 size={15} className="text-emerald-400 shrink-0" />
          <span>Retirada hoje no balcão Cambuí</span>
        </div>
      </div>
      ...
      <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
        {currentList.slice(0, 4).map((product) => {
          ...
          return (
            <div
              key={product.id}
              className="group relative flex flex-col justify-between p-4 rounded-2xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-red-500/40 hover:shadow-xl"
            >
              {/* Badges */}
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-[10px] font-black uppercase tracking-wider">
                  <Flame size={12} />
                  Pronta Entrega
                </span>
                <span className="px-2 py-0.5 rounded-full bg-white/10 text-[var(--home-muted)] text-[10px] font-bold">
                  {activeTab === "pcgamer" ? "Setup Gamer" : "Seminovo Grade A"}
                </span>
              </div>
              ...
              <div className="flex-1 flex flex-col justify-between space-y-3">
                ...
                <div className="pt-2 border-t border-white/5 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    <a
                      href={`https://wa.me/${SITE_CONFIG.whatsapp.number}?text=${encodeURIComponent(`Olá! Vi o produto ${product.name} (${product.price}) no site da Balão da Informática e quero consultar disponibilidade.`)}`}
                      target="_blank"
      ...
      <div className="relative z-10 mt-6 pt-4 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
        <span className="text-xs text-[var(--home-muted)] font-medium">
          {activeTab === "pcgamer"
            ? "Conheça nossas configurações montadas prontas para jogar ou peça sua máquina personalizada."
            : "Mais de 100 opções de notebooks inspecionados para estudo, trabalho ou games."}
        </span>
        <Link
          href={categoryLink}
        ...
        >
          <span>Ver todos os {activeTab === "pcgamer" ? "PCs Gamer" : "Notebooks"}</span>
